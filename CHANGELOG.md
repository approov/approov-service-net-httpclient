...
- **`HandlePinningShouldProcessRequest` is documented as not consulted.** `USAGE.md` listed
  ...
  was one of the pinning bypasses closed in this release, so the hook has no effect here. The bullet
  is removed, the interface member carries an XML doc saying so, and `REFERENCE.md` gained a
